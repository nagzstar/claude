...
`guard-prod.sh` (prod dispatch, prod rerun, direct deploys, force push) and `guard-paths.sh`
(per-role file ownership). It runs in CI on this repo (`.github/workflows/validate.yml`).
...
architectural migration is tier 4 and stops for the user; a prod release request, a third
environment and a paid service are all declined; pure code location goes to `Explore` on
Haiku.
