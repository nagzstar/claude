...
**Moderation is DB-enforced (`20260906220000`).** Only an admin may write a content `status` other than `pending`. Everyone else creates content as `pending` and can only rewrite their own row back to `pending` (edit-and-resubmit). The frontend's `addResponse` sets `approved` only for admins; the policy is what makes that true. **Reuse this `WITH CHECK ... AND status = 'pending'` shape** for any new user-submitted, moderated content, and never add a status-less UPDATE policy beside it.
