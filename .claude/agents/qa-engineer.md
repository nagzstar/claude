...
DEV validation is yours, never the Orchestrator's. Use
`bash "$CLAUDE_PROJECT_DIR/.claude/scripts/dev-probe.sh" matrix <method> <path>` (and `rest`,
`rpc`, `storage`) for every acceptance criterion with a role dimension — it signs in from the
`NGM_DEV_*` variables and prints statuses only; never read `.env`, never write your own probe,
never print a token or key. Report the status matrix (anon / participant / mentor / admin)
verbatim. Never use
