...
call() {  # role method url [json] -> "STATUS digest"
  ...
  printf ' %s\n' "$(node -e 'const fs=require("fs");const d=fs.readFileSync(process.argv[1],"utf8");try{const j=JSON.parse(d);process.stdout.write(Array.isArray(j)?`rows=${j.length}`:String(j.message||j.msg||j.error||j.id||Object.keys(j).slice(0,6).join(",")).slice(0,80))}catch{process.stdout.write(`bytes=${d.length}`)}' "$cache/body")"
}
case "${1:-}" in
